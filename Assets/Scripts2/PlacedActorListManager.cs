...
{
    public Transform contentParent;            // Scroll View's Content
    public GameObject listItemPrefab;          // UI prefab for each actor
    public Button saveButton;                  // Assign in Inspector
    ...
    void Start()
    {
        ...
        // Prioritize newly created module (Create Mode)
        ...
        else if (!string.IsNullOrEmpty(selectedModulePath) && File.Exists(selectedModulePath))
        {
            ...
            // Fix: refresh PlayerPrefs so ARPlacementController gets correct content
    ...
    void SaveModuleData()
    {
        if (string.IsNullOrEmpty(currentModuleName) || environmentData == null)
        {
        ...
        // Save to PlayerPrefs for continuity
